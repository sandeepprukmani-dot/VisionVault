...
def modify_code_for_healing(code):
    ...
    # CRITICAL: Replace 'async with async_playwright() as p:' with direct instantiation
    # This prevents automatic cleanup when the function returns
    modified_code = re.sub(
        r'async with async_playwright\(\) as (\w+):',
        r'\1 = await async_playwright().start()\nglobals()["__p_instance__"] = \1',
        code
    )
    ...
    print("✅ Code transformation: async with removed, browser stays open for healing")
