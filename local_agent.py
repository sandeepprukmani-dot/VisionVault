...
def modify_code_for_healing(code):
    """Modify code to keep browser open and capture page reference for healing using AST."""
    import ast
    import re
    
    try:
        # Use AST to properly transform the code
        tree = ast.parse(code)
        
        class AsyncWithTransformer(ast.NodeTransformer):
            def __init__(self):
                self.found_async_with = False
                
            def visit_AsyncWith(self, node):
                # Check if this is async_playwright context manager
                if (len(node.items) > 0 and 
                    isinstance(node.items[0].context_expr, ast.Call) and
                    hasattr(node.items[0].context_expr.func, 'id') and
                    node.items[0].context_expr.func.id == 'async_playwright'):
                    
                    self.found_async_with = True
                    var_name = node.items[0].optional_vars.id if node.items[0].optional_vars else 'p'
                    
                    # Create replacement statements
                    # p = async_playwright()
                    assign1 = ast.Assign(
                        targets=[ast.Name(id=var_name, ctx=ast.Store())],
                        value=ast.Call(func=ast.Name(id='async_playwright', ctx=ast.Load()), args=[], keywords=[])
                    )
                    
                    # playwright_instance = await p.start()
                    assign2 = ast.Assign(
                        targets=[ast.Name(id='playwright_instance', ctx=ast.Store())],
                        value=ast.Await(
                            value=ast.Call(
                                func=ast.Attribute(value=ast.Name(id=var_name, ctx=ast.Load()), attr='start', ctx=ast.Load()),
                                args=[], keywords=[]
                            )
                        )
                    )
                    
                    # Return the body with the two new assignments prepended
                    return [assign1, assign2] + node.body
                
                return self.generic_visit(node)
        
        transformer = AsyncWithTransformer()
        ...
        if not transformer.found_async_with:
            # Fallback: no async with found, use original code
            modified_code = code
            print("ℹ️  No async with found, using original code")
        else:
            print("✅ AST-based transformation successful")
            
    except Exception as e:
        # Fallback to original code if AST fails
        print(f"⚠️  AST transformation failed: {e}, using original code")
        modified_code = code

    # Replace browser launch references from p to playwright_instance
    modified_code = re.sub(
        r'getattr\(p,\s*browser_name\)',
        'getattr(playwright_instance, browser_name)',
        modified_code
    )

    # Inject page capture after page creation
    ...
    for line in lines:
        ...
        if 'page = ' in line and 'browser.new_page()' in line and not page_captured:
    ...
    modified_code = re.sub(
        r'(await\s+)?browser\.close\(\)',
        '# browser.close()  # Commented for healing',
        modified_code
    )
