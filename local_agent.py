...
async def execute_healing_attempt(test_id, code, browser_name, mode, attempt):
    ...
    try:
        ...
        # 1. Remove all browser.close() calls
        modified_code = modified_code.replace('await browser.close()', '# await browser.close()  # Kept open for healing')
        ...
        # 3. Also prevent close in except blocks by removing those too
        import re
        modified_code = re.sub(r'await browser\.close\(\)', '# browser.close() removed for healing', modified_code)
